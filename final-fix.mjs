/**
 * DREAMORA STAYS — Final Asset Corrections (Wikimedia Commons - Redirects Fixed)
 * Run: node final-fix.mjs
 */
...
const assetsToFix = [
  { file: 'food/food19.jpg', query: 'filetype:bitmap Rasgulla dessert' },
  { file: 'employees/employee4.jpg', query: 'filetype:bitmap Hotel concierge desk professional' },
  { file: 'employees/employee6.jpg', query: 'filetype:bitmap Chef portrait kitchen' },
  { file: 'restaurant/restaurant5.jpg', query: 'filetype:bitmap Luxury restaurant interior dining room' }
];
...
(async () => {
  console.log('\n🔧 DREAMORA STAYS — Correcting remaining assets...\n');
  ...
  for (const asset of assetsToFix) {
    ...
    if (imageUrl) {
      ...
      const dest = path.join(BASE, ...asset.file.split('/'));
